...
5 つのロールセッションは、互いを知りません。別のクローンで動くセッションに「次はあなたの番です」と伝える手段には、いくつか候補がありました。生成AIのセッションどうしの直接通信・サブエージェントによる分担・チャットツールでのメンション、そして GitHub の label です。この章では、なぜ最も地味な label に落ち着いたのかを先に書きます。続いて、その仕組み、それが壊れた 2 つの事故、壊れを検出するテストを扱います。
