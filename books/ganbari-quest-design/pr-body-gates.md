...
![見出しは行全体の完全一致で探す](/images/ganbari-quest-design/pr-body-gates.png)
...
この章の gate を組織の側から見ると、PR 本文が何を担わされていたかが分かります。この製品では、PR を書くのも AI、レビューするのも AI です。人間の組織なら、PR 本文はレビュアへの手紙で、レビュー記録は監査への証跡で、リリースノートは顧客への告知です。この 3 つの文化が、AI の書く 1 つの文書に折り畳まれました。

| 本文の役割 | 誰のためか | 誰が検証するか |
| --- | --- | --- |
| レビューの前提（diff を読む前に持っておくべきこと） | AI レビュア | AI レビュアが読む |
| 人が見た証跡（スクリーンショット、DOM、AC 検証マップ） | 監査とオーナー | 機械が真正性を、人が中身を見る |
| 振り返りの記録（Closes、影響範囲、配布済み env） | 統合 PR の生成、Issue の close | 機械が集約し、突合式で自己検証する |
| 顧客への文（「顧客価値」の第 1 文） | 顧客 | 人が読む |

AI が書き AI がレビューする体制で、人間に残る役割は 2 つに絞られました。証跡が本物かを機械に確かめさせること、そして顧客に届いたかを自分の目で見ることです。前者が Blob SHA の一致と HEAD の実在の検査で、後者が [第Ⅳ部-7](audit-team) の実機監査と [第Ⅵ部-6](skills-as-sop) の read-only の実機確認です。「人がちゃんと見た」証跡は、PR 本文ではなく、統合 PR の merge に付く署名（[第Ⅶ部-3](stacked-pr-integration)）に置きました。本文は AI が書き換えられますが、署名は書き換えられません。

うまくいったかどうかを、組織の視点で書きます。PR 本文を人間組織の文化のまま AI に書かせたことは、半分うまくいきました。顧客価値の 1 文がそのまま告知になる仕組みは、AI が「誰の何が良くなるか」を毎回言語化する契機になり、実装の説明で終わる PR が減りました。半分は、うまくいきませんでした。本文を読む gate は最大の装置群になり、[第Ⅳ部-4](definition-of-done) で見たとおり、AI は本文を「埋める」ことに最適化しました。人間組織の文書は、書く人の常識を前提にしています。AI にその前提は無く、前提の欠落を gate で補うと、gate が組織より大きくなります。
