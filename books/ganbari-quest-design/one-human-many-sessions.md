...
第Ⅳ部は、本書の中心です。ここまでの 3 部で「何を作ったか」を見ました。ここからの 4 部は「どう作ったか」で、生成AIが出す 60 点を 100 点に寄せる工程で、修正が修正を呼ぶのをどう止めたかを追います。第Ⅳ部の 8 章は、そのための体制と判断の話です。

がんばりクエストの開発者は一人です。しかしリポジトリの運用文書には、PO、Dev、QM、監査、Platform という 5 つのロールが登場し、それぞれが別のクローンと別の Claude Code セッションで動いています。この章の問いは、なぜ一人なのにロールを分けたのか、そして分けた境界をどこに引いたか、です。答えは、その体制を定めた「チーム憲章」と、誰が何を決めるかの表にあります。読み終えたとき、一人でロールを分けることが演技ではなく、AI が自分の成果物を自分で承認するのを止める構造だと分かるはずです。
