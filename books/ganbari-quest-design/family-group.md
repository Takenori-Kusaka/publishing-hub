...
家族と認証の群には 2 つの問いがあります。家族というグループに誰を入れ、誰に見せるか。その人が本人だとどう確かめるか。まずは前者です。
...
設計書が「実装済み」と「構想」を表で分け、紹介ページに書いてよい機能を機械的に決めました。設計書の表で実装済みとされるのは、子供の管理と保護者の招待の 2 つです。これに加えて、祖父母に渡す閲覧専用のリンクと、共有端末での子供の切り替えが実装として存在します。
...
設計書は実装済みと構想を表で完全に分けます。実装済みは実装ファイルの参照付きで、こどもの管理（追加、編集、年齢帯、テーマの色、アーカイブ）とメンバーの管理（招待、役割）の 2 つ。構想は家族グループの招待リンク、テナント名の設定、家族プロフィールの 3 つで、「紹介ページ、法務文書、アプリの画面に『実装済み』と書いてはいけない」と明記されています[^familydoc]。この分離は [第Ⅰ部-1](product) で見た製品方針の、設計書側の実装です。
...
1 つ目はトークンです。招待コードは 32 バイトの乱数から作り、データベースには SHA-256 のハッシュだけを保存します。生のトークンは作った時に呼び出し元へ返すだけです。持っているだけで通る合言葉になって機密性が退行するのを防ぐためで、トークン自体が推測できない 256 ビットの乱数なので追加の乱数（ソルト）は不要、パスワードと違い辞書攻撃が成立しない、と設計判断が書かれています。新しい依存は足さず Node.js の暗号の標準機能だけで、リポジトリ内の既存の慣行に揃えています。閲覧リンクのトークンの生成と、おやカギコードを再設定するときの一時パスワードを処理時間から中身が漏れない方法で比べることの 2 つです[^invitetoken]。
...
**狙い。** 取得関数の結果を加工せず画面に渡し、コードを短く保つ狙いでした。

**起きたこと。** 2026 年 9 月の監査で、ポイントの表示が「[object Object] ポイント」になっている不具合が見つかりました[^viewpage]。

**なぜ。** 戻り値が複数の型のいずれかになる関数の結果を、型を絞らずに画面へ渡していたからです。片方の型が画面に漏れました。
...
子供を選ぶ操作には、もう 1 つの意味があります。親が子供の画面へ戻った瞬間に、おやカギコードのセッションのクッキーを削除します。次に子供が親の画面へ行こうとすると、再びおやカギコードを要求されます。[第Ⅱ部-9](auth) で見るおやカギコードの関門は、この「切り替えで失効する」設計が核心です[^security]。
...
閲覧専用リンクの `[object Object]` は、型が緩いところで起きる典型です。実機の監査で見つかったことも、[第Ⅳ部-7](audit-team) で見る「機械は決めた条件しか見ない」の実例です。
