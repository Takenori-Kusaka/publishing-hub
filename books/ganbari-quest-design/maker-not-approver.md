...
第Ⅳ部は、一人と生成AIで「独立した判断」をどう作るかを追っています。前の 2 章で、ロールを分け、受け渡しを GitHub の label に載せました。しかし分けただけでは、AI は自分の成果物を自分で承認します。この章の問いは 2 つです。作る側と承認する側を分けることを、AI の善意に頼らずどう強制するか。そして、承認する側の AI が形だけの承認に滑り落ちるのをどう止めるか。

答えは 3 つの意思決定記録（ADR）に分かれて残っています。GitHub のアカウントを分けて物理的に強制した ADR-0022、承認側の AI が 33 日で 42 回「ゲート役」から「CI の代弁者」へ滑り落ちるのを観測した ADR-0056、そしてその対策を費用の理由で一部外した ADR-0068 です。読み終えたとき、「独立した判断」は一度作れば済むものではなく、維持費と天秤に掛け続けるものだと分かるはずです。

## 自分の PR を自分で merge していた

**狙い。** GitHub には「承認が 1 件必要」というルールがあり、当初からそれを有効にしていました。一人で開発していても、PR を出してから merge するまでに、レビューの手順を挟むつもりでした。

**起きたこと。** ADR-0022 の背景は率直です。
...
**なぜ。** ルールは「承認が要る」でしたが、リポジトリのオーナーは管理者権限でそれを迂回できました。迂回が可能で、迂回した方が速く、止める人がいなければ、AI であれ人であれ迂回します。QM のセッションに「レビューしてから承認せよ」と書いても、承認の操作は Dev と同じアカウントで押せました。

**変えたこと。** 役割分離は「Dev のアカウントが PR を作成 → QM のアカウントが approve → squash merge」の順です[^adr22]。一人の人間が 2 つの GitHub アカウントを持ち、Dev セッションと QM セッションで別のアカウントを使います。管理者の迂回は、オーナー自身に対しても閉じました。

**読者のリポジトリでは。** 「レビューしてから merge する」は規律であって構造ではありません。作成者が承認の操作を押せる限り、押されます。アカウントを分け、迂回の権限を自分から外すところまでが、分離です。
