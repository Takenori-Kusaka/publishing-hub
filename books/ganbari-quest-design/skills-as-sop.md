...
古びるのは数字です。cost-review の 1,500 円と DynamoDB は、書いた日の正解でした。skill に数字を書かず、SSOT（事業計画書、cost-audit の run log）を指すべきでした。[第Ⅵ部-1](claude-md-hierarchy) の「掲載しない」は、skill にも適用できます。

もう 1 つ、skill は増える一方でした。26 本のうち削除されたものはありません。cost-review のように「読むと間違う」skill は、直すか消すかのどちらかです。[第Ⅵ部-3](adr-deletionism) の削除主義を、skill にも掛けるべきです。
