...
この本を書くために原典を読み直し、リポジトリの側の不整合を 40 件ほど見つけました。設計書が実装より先に走った箇所、撤去したテーブルを読み続ける workflow、初版のまま凍った AI 向けの文脈ファイル、30MB の生成物が 24 版積まれた履歴。どれも「動いている」ので、CI では落ちません。

本の初版には、その一覧を表として載せていました。この改訂で表は外し、すべて GitHub の Issue にしました。理由は 2 つです。本に「直す予定のこと」を書くと、直した瞬間に本が古くなること。そして、直す判断はこの本の読者ではなくリポジトリのオーナーのものであること。宿題の一覧は、リポジトリの Issue を `type:docs` と `type:refactor` で絞れば読めます。

宿題から学んだことは、1 つだけ書いておきます。動いているものは落ちない。だから、本を書くために原典を全部読み直すまで、40 件は見つかりませんでした。[第Ⅴ部-4](fitness-functions) の「記録を反証可能にする」は、こういう残骸のためにあります。ただし 40 件すべてに検査を置くのは、[第Ⅳ部-8](platform-session) の「装置を増やさない」に反します。私は増やさない方に賭け、月 1 の棚卸で人が読み直す方を選びます。
