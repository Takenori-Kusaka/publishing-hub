...
Aurora DSQL の構成は警報を 2 本に限定しています。`TotalDPU` が日次 3,225（月 10 万のペース）を超えたら。`ClusterStorageSize` が 0.8 GiB を超えたら。観測の指標は警報ではなくダッシュボードで見て、無料枠の 10 本を温存します。予算の警報は 1 ドルで、実質「課金が発生したら知る」設定です[^dsqlstack]。[第Ⅲ部-1](serverless-cost) で見るとおり、3 か月の Aurora DSQL の請求は 0 ドルでした。
