...
入口の防御の検査は、[第Ⅲ部-2](cdk-stacks) の共有の秘密の値が本当に効いているかを見ます。本書では、CloudFront を通らない直接の呼び出しを拒む検査を入口の防御と呼びます。稼働確認の入口はこの防御の対象外なので必ず 200 を返し、「検査が黙って無効」を検出できません。そこでヘッダ無しで管理画面を叩き、404 が返ることを確認します。CloudFront 側のヘッダが Lambda 側の環境変数とずれる方向は、本番の CloudFront が日本国外の実行機を 403 で弾くため自動化できず、受け入れる残課題として明記されています[^deployyml]。
...
一方で、1,200 行の自動処理は装置です。[第Ⅳ部-8](platform-session) の凍結以降、デプロイの自動処理に足されたのは入口の防御の検査と環境変数のずれの検査で、どちらも「顧客の金かデータに現に届いている」例外に当たります。承認が先頭のコミット 1 つに紐づく仕様は、知らなければ 2 度止まります。生成AIにデプロイの自動処理を書かせるとき、最も読ませるべきは設計判断の記録の本文ではなく、記録に追記された「運用で分かった性質」の節でした。
...
[^deployyml]: 本番デプロイの自動処理（約 1,200 行）。デプロイの各段階、稼働確認と巻き戻し、入口の防御の検査と残課題、管理から外れた資源の検出、環境変数のずれの検査、後続の本番の画面操作テストとリリースの処理。出典: [.github/workflows/deploy.yml](https://github.com/Takenori-Kusaka/ganbari-quest/blob/3af6c2ed9fd4fe5766fc80c255656e940f8ec8f0/.github/workflows/deploy.yml)
