...
入口の防御が効いているかの検査は、[第Ⅲ部-2](cdk-stacks) の共有の秘密の値を確かめます。稼働確認の入口はこの防御の対象外なので必ず 200 を返し、「検査が黙って無効」を検出できません。そこでヘッダー無しで管理画面を叩き、404 が返ることを確認します。CloudFront 側のヘッダーが Lambda 側の環境変数とずれる方向は、本番の CloudFront が日本国外にある GitHub Actions の実行機を 403 で弾くため自動化できず、受け入れる残課題として明記されています[^deployyml]。
