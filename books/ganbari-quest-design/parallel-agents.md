...
ロールを分け、受け渡しを決め、完了の定義を決めたあとに残る誘惑は、速くすることです。この章の問いは、生成AIのセッションを並列にすると何が速くなり、何が壊れるか、です。

生成AIのセッションは、subagent を起動して仕事を分担できます。並列にすれば速くなるように見えます。この章では、がんばりクエストがどこまで並列化を許し、どこで禁じたか、そして並走が実際に何を壊したかの実測を扱います。結論を先に書くと、並列化で速くなるのは「読む・調べる・書く」だけで、検証は速くならず、teammate の報告は届かないことがあります。読み終えたとき、teammate の報告を信じずに diff を見る習慣の根拠が分かるはずです。
...
lock の実装にも落とし穴がありました。狙い・起きたこと・原因・変えたことの順で書きます。

**狙い。** lock の持ち主をプロセスの ID で記録し、持ち主が生きている間だけ lock を有効にするつもりでした。持ち主が死ねば lock は stale として奪えるので、セッションが落ちても lock が残り続けることはない、という設計です。

**起きたこと。** 実測では、同じセッションの lock 5 本がすべて別の持ち主 ID を記録し、取得の 1 分後には全て「死亡」と判定されていました。再入の判定が効かず、解放が常に no-op になり、lock が取得直後から失効する。三重の破綻で「排他はまったく成立していなかった」と文書は記しています[^concurrency]。

**なぜ。** 持ち主として記録していたのは hook の親プロセスで、それは呼び出しごとに生成と消滅を繰り返す短命なプロセスでした。「持ち主が生きているか」の判定は毎回「死んでいる」になり、lock は毎回奪えました。hook は入っていて、動いていて、何も守っていませんでした。

**変えたこと。** 祖先のプロセスツリーを辿って、常駐のセッションプロセスを持ち主にしました。「hook が入っている」ことと「排他が効いている」ことは別で、後者は実測でしか確かめられません。

**読者のリポジトリでは。** 排他や lock を自作したら、「2 本目が本当に止まるか」を 1 度は同時に走らせて確かめてください。止まらない lock は、止まる lock と同じ log を出します。
