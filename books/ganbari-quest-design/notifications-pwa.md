...
先に、通知がどこを通って端末に届くかを書きます。サーバレスの構成では、常駐するプロセスがありません。それでも Web Push は送れます。

Web Push は、ブラウザごとの push service（Chrome なら Google の FCM、Firefox なら Mozilla のサービス）に HTTP で POST する仕組みです。アプリは `web-push` ライブラリで、環境変数に配った VAPID の鍵で署名した POST を、購読時に保存した endpoint の URL へ送ります。SNS や Pinpoint のような AWS の通知サービスは使いません。受け取る側は Service Worker の push イベントで、ブラウザが閉じていても OS の通知として出ます[^notifservice]。

送る契機は 2 つあります。達成の通知（レベルアップ、実績）は、活動を記録したリクエストを処理している Lambda の中で、記録と同じ処理の流れの中から送ります。その時点で Lambda は起動済みなので、cold start は関係ありません。リマインダーとストリークの警告は、時刻で決まるので、[第Ⅲ部-3](lambda-sveltekit) で見た cron の経路を通ります。EventBridge が 15 分ごとに dispatcher の Lambda を起こし、dispatcher が Function URL の配信 endpoint を呼び、SvelteKit の Lambda が送信の対象を選んで POST します。この Lambda が cold なら起動して送るので、遅れは数秒で、送られないことはありません。

弱点は cold start ではなく別のところにあります。リマインダーの粒度が 15 分であること、1 回の cron の処理予算が 30 秒であること、そして cron が届いていなければ何も送られないことです。最後のものは実際に起きました。[第Ⅴ部-4](fitness-functions) で見たとおり、cron は 4 か月間 401 で失敗し続け、その間リマインダーは 1 通も送られていません。
