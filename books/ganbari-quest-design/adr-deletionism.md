...
「常時参照は 10 本まで」が守れなかった理由を、もう 1 段掘ります。10 本という数は、人の記憶の限界から来ています。しかし ADR の数は、人の記憶ではなく開発対象の規模から来ます。このリポジトリは [第Ⅶ部-1](monorepo-and-artifacts) で見るとおりモノレポです。アプリ・CDK・LP・設計書・テストの 12 のトップレベルを、1 つの ADR 一覧が受けています。課金の ADR、DSQL の ADR、LP の ADR、品質ゲートの ADR は、それぞれ別の読み手が別の場面で参照しますが、一覧は 1 つです。規模に比例して ADR は増え、一覧の上限は守れません。

選択肢は 2 つありました。1 つは、リポジトリを小さくすることです。ADR の一覧を分ける単位をリポジトリにすれば、1 つの一覧は 1 つの領域だけを持ちます。もう 1 つは、ADR を平らな一覧ではなく、ツリーで索引できる形にすることです。部やサブシステムごとに枝を持ち、読み手は自分の枝だけを辿る。CLAUDE.md が [第Ⅵ部-1](claude-md-hierarchy) で 7 階層になったのと同じ形を、ADR にも適用する案です。

どちらも、この 7 か月では採りませんでした。モノレポは [第Ⅶ部-1](monorepo-and-artifacts) で見るとおり、一人と AI の開発では分ける利益が無く、ツリー構造の索引は、削除主義で本数を絞る方が先だと判断したからです。結果として 37 本が 1 つの一覧に並び、AI は必要な ADR を探すのに一覧全体を読みます。同じ形の AI 開発を始める人が最初に踏むのは、たぶんここです。ADR の上限を決める前に、上限を守れる単位（リポジトリか、索引の枝か）を決める必要があります。
