...
```mermaid
flowchart TD
    P[課題：n種類の財を<br>どの用途に配分するか]
    P --> G
    subgraph G[配分に必要な情報量]
        direction TB
        M1["方式1. 直接交換<br>交換比率 n(n-1)/2 通り<br>財が増えると急速に破綻"] ~~~ M2[方式2. 中央計画<br>全需給情報を中央へ集約<br>局所知識が集約で欠落]
        M2 ~~~ M3[方式3. 価格機構<br>局所知識を n 個へ圧縮<br>各主体は周辺のみ観測]
    end
...
この図は、最上段の1つの課題への3つの回答を、枠の中で縦に並べて比較するものです。3つの箱は同じ課題を解く方式の並列で、上下の位置は優劣を意味しません。

各箱の2行目が、その方式で扱わなければならない情報の量です。3行目が、その情報量から生じる制約です。この図が示すのは、資本主義の代替されにくさが道徳的な優位や政治的な力関係ではなく、情報処理の構造に由来するという論点です。

枠内の1段目「直接交換」は、財の種類が増えるにつれて必要な情報量が二乗で増加します。

2段目「中央計画」は、必要な情報を1か所へ集める設計です。この方式の制約は、計算能力ではありません。集約の過程で、集計になじまない局所知識が失われることにあります。ある現場の担当者が持つ「この機械は今日調子が悪い」という知識は、報告書式に載りません。

3段目「価格機構」は、情報を集約しません。各主体は自分の周辺の価格だけを観測し、それに応じて行動します。全体の需給状況は誰も把握していないまま、価格の変動を通じて調整されます。

ここで、前章の論点が再び現れます。貨幣が「それ自体としては何の価値も持たない」という性質は、この圧縮を可能にする条件です。もし貨幣が特定の実体と結びついていれば、その実体の性質が変換に制約を課します。

なお、この図は3段目の方式が優れていることを示すものではありません。価格機構は、貨幣建てに変換できない情報を構造的に無視します。[第Ⅲ部](future-method)および[第Ⅳ部](implication-state)では、この無視される領域が持つ意味を扱います。
...
3.  **現代社会の現状と課題**
    ...
    本章の枠組みは、[第Ⅱ部](japan-current-state)および[第Ⅲ部](future-method)で次の2点に適用されます。
