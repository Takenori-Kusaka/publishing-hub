...
この最後の点は、経済学において外部性として扱われます。汚染の費用が価格に反映されない場合、汚染は過剰に発生します。[第Ⅰ部C-6](05_cholera-outbreak-sewage)で記述したテムズ川の汚染は、この構造の事例です。
...
この論点は、[第Ⅰ部B-5](socialism-communism-history)で記述した計画経済の観測と同じ方向を示します。生産の指標が物量で与えられた場合、生産単位は指標の達成を優先し、品質や需要への適合を軽視する傾向を示しました。
...
計量できない場合は、2つ目の問いに進みます。排除はできるなら左の方式2です。計量はできないが利用の可否は制御できる場合で、有料道路や会員制の施設がこれにあたります。
...
[第Ⅰ部B-3](capitalism-why-irreplaceable)で整理したハイエクの議論に照らせば、この点は本質的です [ [ 83 ] ](https://www.aeaweb.org/aer/top20/35.4.519-530.pdf) 。価格機構が有効なのは、分散した局所知識を価格という1つの数値に圧縮できる範囲においてです。圧縮の対象とならない情報は、この機構の外側に残ります。
...
3.  **現代社会の現状と課題**
    ...
    [第Ⅲ部](future-method)および[第Ⅴ部](42_taoist-wu-wei-non-action)では、この論点を扱います。とりわけ、災害時の相互扶助の能力や技能の継承は、計量が困難でありながら継続能力に直結します。これらをどう扱うかが設計上の課題です。
