...
flowchart TD
    U[利用者]
    U --> GU
    subgraph GU[規定されなかった層：集中]
        direction TB
        L4[層4. 情報の探索と提示<br>検索と推薦<br>現状：少数の事業者へ集中] ~~~ L3[層3. 情報の所在と管理<br>事業者の設備上に蓄積<br>現状：少数の事業者へ集中]
    end
    subgraph GD[規定された層：分散]
        direction TB
        L2[層2. 相互接続の規約<br>網の中核は単純に保つ<br>現状：公開規約として維持] ~~~ L1[層1. 物理的な通信経路<br>複数経路による冗長性<br>現状：分散が維持]
    end
    L3 ~~~ L2

    style U fill:#292524,stroke:#a8a29e,color:#fafaf9
    style GU fill:#fef2f2,stroke:#dc2626,color:#1e293b
    style GD fill:#ecfdf5,stroke:#10b981,color:#1e293b
...
この図は、4つの層を上下に積み重ねたものです。最上段の利用者に近い層ほど上に、物理的な通信経路ほど下に置いています。矢印は利用者がどの層に直接触れるかを示し、層の間の順序は積み重ねの順序です。

上の枠に入る層3と層4は、設計が何も規定しなかった層です。ここで集中が形成されました。下の枠に入る層1と層2は、設計が明示的に規定した層です。物理的な通信経路は複数存在し、規約は公開され、誰でも実装できます。

上下の枠を分けて示した理由は、分散と集中が異なる層で同時に成立しているという構造にあります。上の枠の集中は、下の枠の設計に違反して生じたのではありません。設計は、これらの層について何も規定していませんでした。

この構造から抽出できるのは、次の一般則です。分散設計は、それが明示的に規定した層においてのみ分散を保証します。規定されなかった層では、経済的な誘因に従って構造が決まります。

そして、最上段の利用者から矢印が向かう先は上の枠です。利用者にとっての実質的な依存は層3と層4に存在します。通信経路が冗長であっても、情報の所在を管理する事業者が停止すれば、利用者はその情報へ到達できません。

この論点は、本書が[第Ⅴ部](42_taoist-wu-wei-non-action)で扱う自律分散型システムの設計に直結します。ある層で分散を実現しても、規定しなかった層で集中が再形成されるならば、全体としての耐性は向上しません。
...
2.  **歴史的事実の本質**
    ...
    この機序は、インターネットに固有のものではありません。[第Ⅰ部C-1](innovation-overview)で整理した汎用技術の性質、すなわち補完的な革新の誘発という性質が、集中を生む方向にも作用します。

3.  **現代社会の現状と課題**

    [第Ⅱ部](japan-current-state)では、この集中が現在どの程度進行しているかを扱います。[第Ⅲ部](future-method)では、集中した層に障害が生じた場合の影響を、シナリオとして評価します。

    [第Ⅴ部](42_taoist-wu-wei-non-action)では、層3と層4における分散をどう設計しうるかを検討します。そこでの要件は、通信経路の冗長性ではなく、情報の所在と探索を利用者側に保持する仕組みです。
