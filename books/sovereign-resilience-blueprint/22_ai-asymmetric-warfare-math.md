...
防御の成否を規定する2条件と、条件2を規定する制約を示します。

```mermaid
flowchart TD
    D[防御の持続に必要な2条件]
    D --> C1[条件1. 迎撃の成功率<br>技術の問題<br>現行は高い水準]
    D --> C2[条件2. 消費 ＜ 補給<br>在庫と生産の問題<br>消費が補給を下回ること]
    subgraph G[速度比を規定する制約]
        direction TB
        K1[制約1. 搭載数の上限<br>洋上再装填は実証段階] ~~~ K2[制約2. 生産速度<br>調達期間が長い]
        K2 ~~~ K3[制約3. 攻撃側の生産<br>汎用部品で短期間]
    end
    C2 -->|攻撃側の生産が速い場合| F[帰結. 在庫の単調減少<br>成功率が高くても<br>いずれ交戦不能]
    C2 --> G
    ...
    style K3 fill:#3f1d1d,stroke:#dc2626,color:#fef2f2
    style G fill:#f8fafc,stroke:#94a3b8,color:#1e293b
```

#### 図の読み方

この図は、上段の1つの枠から左右2つの条件へ分かれ、右の条件2からさらに下の左右2つへ分かれる形をしています。右下の枠内には、条件2を規定する3つの制約が縦に並びます。

この図が示すのは、防御の評価軸が交戦単位ではなく時間軸にあるという点です。

左の条件1だけを見れば、防御は成功します。1回の交戦を切り取れば、命中率は高く、標的は撃破されます。この観点では、防御は機能しています。

しかし、右の条件2を含めて交戦を繰り返す場合、評価は変わります。消費が補給を上回る限り、在庫は単調に減少します。減少が続けば、いずれ交戦を継続できなくなります。条件2の左下の枠が、この帰結です。

右下の枠内の3つの制約は、補給速度と攻撃側の生産速度の比を規定します。搭載数の上限と生産速度が防御側の制約、汎用部品による短期間の生産が攻撃側の条件です。

この構造は、[第Ⅰ部D-2](war-technology-evolution)で整理した費用構造の観点と重なります。そこで述べたとおり、軍事技術の社会的な帰結を規定するのは破壊力ではなく、それを保有し運用するために必要な資本と組織です。

同時に、この構造は[第Ⅱ部D-5](26_power-grid-cascade-collapse)で記述する電力系統の連鎖停止とも同型です。個々の動作は正常でありながら、系全体としては望ましくない方向へ進行します。

なお、この構造は防御が無意味であることを意味しません。意味するのは、防御の設計において、単発の交戦性能ではなく、補給を含めた持続能力が評価軸になるということです。

[第Ⅱ部B-1](report-ukraine-war)で参照した分析が、長距離火力による兵站への脅威を主導権の転換点として記述していたことも、同じ論点を示しています。
...
2.  **歴史的事実の本質**
    ...
    この一般則は、軍事に限られません。情報セキュリティにおいても、攻撃の試行が安価で、防御の検証が高価である場合、同じ構造が生じます。[第Ⅲ部-4](risk-ai-and-dependency)で述べる情報の生成と検証の非対称も、同型です。

3.  **現代社会の現状と課題**

    日本の防衛および国民保護の想定について、本書は[第Ⅰ部D-5](war-blitzkrieg-doctrine)の章末で、短期間の事態を前提としてきたと評価しました。これは著者の評価であり、公的文書の記述ではありません。国民の保護に関する基本指針は、着上陸侵攻の場合に措置の期間が比較的長期に及ぶと記述する一方、事態の想定期間や備蓄の日数を定めていません [ [ 424 ] ](https://www.kokuminhogo.go.jp/pdf/260331shishin.pdf) 。

    本章の構造に照らすと、この前提には検討の余地があります。持続能力が評価軸となる場合、備えの期間設定が決定的になるためです。[第Ⅲ部](future-method)および[第Ⅴ部](42_taoist-wu-wei-non-action)では、この期間設定を具体的に扱います。
