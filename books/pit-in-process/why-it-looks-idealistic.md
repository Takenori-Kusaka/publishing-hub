...
```mermaid
flowchart TD
    subgraph AI_side["AIが解こうと<br/>していること"]
      A1["実装コストのゼロ化"]
      A2["生成の高速化"]
      A3["自己検証"]
    end
    subgraph H_side["人間・組織に<br/>残ること"]
      B1["価値判断・言語化<br/>・検証の能力"]
      B2["決定権限と説明責任<br/>の一意性"]
      B3["暗黙知の表出化"]
    end
```
...
```mermaid
flowchart TD
  X["PO + AI で<br/>チームが回る"] --> A["前提A:<br/>実装が自動化される"]
  X --> HR["前提B・C・D:<br/>人間と組織に残る課題"]
  HR --> B_C["前提B・C:<br/>POが要求・検証できる"]
  HR --> D["前提D:<br/>決定権限がPOに集約"]
  A -.->|AIが解く| Y["解決済みへ向かう"]
  B_C -.->|人に残る| Z["残る"]
  D -.->|組織に残る| Z
```
