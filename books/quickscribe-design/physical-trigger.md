...
> 個人開発OSS「QuickScribe」（ローカル完結ボイスジャーナル）の設計連載の一章です。ここまでアーキテクチャ・整形・プライバシー・データ設計と、中身の話をしてきました。今回は入口の話、つまり物理ボタンひとつで「話す→整形→残す」を摩擦なく始める体験をどう設計したかです。コードは v1.0.0 時点。設計判断は該当箇所を引用し脚注で出典（ADR）を示します。
...
次章では、この入口から入ってきた音声の「精度」を、なぜ競争軸にしないと決めたか ― 文字起こしをコモディティ扱いして回帰監視に落とす設計を書きます。

[← 前の章](vault-data-design) ／ [次の章 →](accuracy-as-commodity)
