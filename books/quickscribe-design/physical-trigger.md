...
> 個人開発OSS「QuickScribe」（ローカル完結ボイスジャーナル）の設計連載の一章です。ここまでアーキテクチャ・整形・プライバシー・データ設計と、中身の話をしてきました。今回は入口の話、つまり物理ボタンひとつで「話す→整形→残す」を摩擦なく始める体験をどう設計したかです。コードは v1.13.0 時点。設計判断は該当箇所を引用し脚注で出典（ADR）を示します。
...
- **特定デバイスに縛られない**。「このアプリは Stream Deck 専用」にはしたくない。
...
加えて、**CLI からも起動できる**ようにしました。`quickscribe --toggle-record` のようなコマンドで、single-instance の仕組みを使って実行中のプロセスへ転送します。LogitechやRazerのように任意コマンドを起動できるデバイスは、こちらを直接叩けます。ホットキーとCLI、2つの入口を用意して、デバイス側の得意な方へ合わせられるようにしました。

### 主要な設計改修：録音APIを start/stop に分離する
...
そこで、**録音APIを内部で明示的な start と stop に分離**しました。トグルは「今録音中なら stop、そうでなければ start」という上位の組み合わせになります。momentary は「押されたら start、離されたら stop」という組み合わせです。**下位に start/stop を持てば、トグルと momentary をどちらも上位で構成できる**わけです。
...
押している間だけの録音は、グローバルショートカットの**押下**（Pressed）と**離し**（Released）を拾って実現します[^shortcut]。停止時には短い **release delay**（末尾切れ防止）を入れます。指を離した瞬間に切ると、最後のひと言が欠けがちなので、少しだけ録音を延ばす。これはOBSやDiscordでも使われる定石です。
...
グローバルホットキーの登録はバックエンドで行います[^shortcut]。設定されたアクセラレータ（例: `F13`）を解釈し、既存の登録を解除してから登録し直す。パースや登録の失敗は、ユーザー向けの安定エラーコードに変換します（エンジン抽象の章で触れたエラー設計と同じ流儀です）。
...
`start`/`stop` という下位の分離があるから、`Toggle` と `Momentary`、そして2つの入口が、同じ土台の上に乗る。この一段の抽象がこの章の要点です。

### 即時性のために、常駐メモリを受容する

この章の設計には、あとから金額のはっきりした請求書が届きました。**トレイに常駐しているあいだ、WebView2 を破棄せず保持する**という判断です[^adr33]。常駐リソースを調べた際に、アイドル時のメモリが実測で約195MBだと分かりました。ウィンドウを閉じたときに WebView2 を捨てれば、そのうち165MBほどは回収できます。

それでも保持を選びました。理由は、この章の主題そのものです。押した瞬間に録れることが差別化軸で、破棄すると復帰時に WebView2 の起動とフロントエンドの再初期化が要ります。**削ってよいのはコモディティ側のコストであって、差別化軸ではない**、という線引きです。録音経路だけ Rust 側に寄せて UI を捨てる案も検討しましたが、録音開始のフィードバックまで含めた体験の一貫性が崩れるため却下しました。

正直に書くと、これは「最適化しなかった」判断の記録です。回収できるリソースの大きさより、失う体験のほうが高いと判断しました。アイドル時メモリの目標が300MB目安で、実測が目標内だったことも後押ししています。数字が目標を超えたら、この判断は再検討の対象になります。
...
一番の学びは、**統合体験は「自分で作る」より「プラットフォームの公式手段に乗る」ほうが広く強い**ということです。Stream Deck 専用の凝った連携を作り込むより、「任意のホットキーに割り当ててください」というガイド1枚のほうが、結果的にあらゆるデバイスに対応できました。差別化＝独自実装、とは限りません。既存の公式手段を正しく前提にする設計判断も、りっぱな差別化です。
...
最後に正直な弱点を1つ。ボタン面での録音状態の視覚フィードバックや、Stream Deck 上でのmomentary保持といった、より凝った連携（カスタムプラグイン）は**まだ作っていません**。これらが本当に必要になったら、今の start/stop の土台の上に足す計画です。「必要になるまで作らない」を、ここでも通しています。

次章では、この入口を軽くした結果その下流で起きたこと ― 文字起こしが終わる前に次を録ってしまう問題を、逐次キューでどう受け止めたかを書きます。

[← 前の章](vault-data-design) ／ [次の章 →](background-jobs)


[^adr33]: ADR-0033「トレイ常駐時に WebView2 を破棄せず保持する（常駐メモリ約 195 MB の受容）」。Status は Proposed。破棄すれば165MB を回収できるが、復帰時の再初期化が物理ボタンの即時性を損なうため受容する判断と、却下案（hide 時の破棄・録音経路のみ Rust 側へ寄せる）が記録されている。出典: [docs/adr/0033-keep-webview2-alive-in-tray.md](https://github.com/Takenori-Kusaka/QuickScribe/blob/main/docs/adr/0033-keep-webview2-alive-in-tray.md)

[^adr14]: ADR-0014「物理トリガー連携」。独自デバイスAPIを作らず、各エコシステムの公式手段（Stream Deck 組み込み Hotkey / Logi / Razer / HIDフットスイッチ）へ橋渡しする方針。加えて、F13–F24推奨、録音APIをstart/stopに分離してmomentary を後付け可能にする、既定はトグルのまま、という判断も含む。出典: [docs/adr/0014-physical-trigger-integration.md](https://github.com/Takenori-Kusaka/QuickScribe/blob/main/docs/adr/0014-physical-trigger-integration.md)
