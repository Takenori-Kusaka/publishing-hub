---
title: "競合の真ん中が無人な理由 ― そして、その空白は縮みつつある"
---
...
作ってから競合をあらためて調べ直して、むしろこちらのほうが面白いと感じたことがあります。「似た製品がない」こと自体より、なぜこの組み合わせを誰も作っていないのか、という問いです。この章はそれを追いかけます。先に結論を言うと、この空白は「つい最近まで技術的に不可能だった」ことと、「可能になった今もクラウド課金の経済がこの形を作らせない」ことの二重の壁で、かなりよく説明できます。そして掘り下げるほど、その同じ壁が自分のプロダクトの弱点そのものだと分かってきました。

## 何を作ったのか

QuickScribe は、話した言葉を思考の整理に変えるための音声ジャーナルです。使い方はシンプルです。話して、端末の中で文字起こしして、端末の中の AI で「要約せずニュアンスを残したまま」整える。これを日記として端末に残すだけです。既定の状態では、音声とテキストのどちらも端末の外に出ません。技術的には Tauri 2 のデスクトップアプリで、Rust のバックエンドと Svelte のフロントエンドでできています。
...
というわけで、厳密な意味での空白とは言えなくなりました。それでも「要約せずニュアンスを残す整形 × 完全ローカル × 日記特化」という真ん中は、2026年時点でまだ無人だと考えています。そして隣接する3社がじわじわ近づいてきているので、この空白はむしろ縮みつつある、というのが今の見立てです。
...
もう1つの柱がこれです。端末上で質の高い整形をこなせる小型 LLM と、実用的なローカル文字起こしが同時に揃ったのは、ざっくり2024年から2026年にかけてです。それ以前はそもそも技術的に成立しませんでした。依存なしで CPU でも動く whisper.cpp が実用域に入り、数 GB の量子化モデルが普通のノート PC で「整形」に足る品質を出すようになった。つまり、この組み合わせは「作らなかった」のではなく「作れなかった」時期が長かった、という見方です。

これが本章のなかで中心的な「なぜ今か」でもあります。棄却する証拠があるとすれば「2020年ごろにも同等品質のローカル整形が可能だった」という事実ですが、今のところ見当たりません。ただし、日本語で「ニュアンスを残した整形」の品質がクラウドと並ぶかどうかは、公開されたベンチマークが乏しく、私自身まだ胸を張れる段階ではありません。ここは推測混じりだと正直に書いておきます。

### 仮説3：「要約して捨てる」は数字にできるが、「残して育てる」は測れない（説得力：中〜強）

要約は「週に何時間の短縮」という売れる指標を作れます。一方で、ニュアンスを残した内省の価値は主観的で、言語化とマーケティングと計測のどれも難しい。だから多くのプロダクトが、測れて売れる「要約」のほうへ流れていく、という仮説です。内省系のアプリが継続率の壁（30日後に数％まで落ちる、というのはこの領域でよく言われる水準です。出典を特定できた数字ではなく、未検証の見立てとして書きます）に晒されやすいことも、これを補強します。この非対称は、あとで自分に跳ね返ってきます。

### 仮説4：日記は「事業」ではなく「機能」にされがち（説得力：中）

当初は「日記の市場が薄いから大手が来ない」と考えていましたが、これは半分外れです。市場規模の推計は調査会社によって2〜3倍ばらつき、あまり当てになりません（個別の推計値は照合できていないため、ここでは数字を挙げません）。しかも Apple は iOS に Journal を、Microsoft はディクテーションを、それぞれ軽い機能として載せています。「市場が絶対的に小さいから触らない」のではなく、「日記は単体で収益化しにくいので、本気の事業ではなく機能として軽く触れられる」と言い直すほうが正確でした。
...
一番効いたのは、価値の本体でない部分を差し替え可能な境界として切り出したことです。文字起こしエンジンと整形エンジンをそれぞれ trait（`TranscriptionEngine` / `FormattingEngine`）として定義し、設定から実装を選ぶファクトリ越しで差し替えられるようにしました。おかげで、既定をクラウドからローカルへ動かす判断を、新しいプロバイダの trait 実装を足すだけで入れられました。整形の既定をローカルの Ollama にし、日本語の文字起こし既定を kotoba-whisper にしたのがその判断です[^adr21]。「ローカル完結を既定にする」を、あとから設計に無理なく差し込めたのは、この境界のおかげでした。

なお、この日本語の文字起こし既定は、公開後の実測でのちに覆されます。その経緯は実測編の「既定モデルを実測で覆した日」で書きます。
...
ここまで「なぜ誰もやらないのか」を偉そうに論じてきましたが、その論の刃は自分にも向きます。掘り下げる過程で見つかった、QuickScribe 自身の未解決を隠さずに書きます。むしろこの節が、この章で一番読む価値のあるところだと思っています。
...
- **肝心の「ニュアンス保持」を測れていない**。これはコア価値そのものなのに、客観指標がありません。仮説3で「内省の価値は測れない」と偉そうに言いましたが、自分もその通り免れていない。第三者から「本当に要約と違うのか」と問われて、数字では返せないのが現状です。ここは、この本を書こうとしたことで自分でも再認識した宿題で、どう測るべきかを腰を据えて考えているところです。
...
次章では、その言葉をどう構造に落としたか ― アプリ全体のアーキテクチャと、本書の各章がその構造のどこを扱うのかを示します。
