...
この記事の作成には、生成AIの Claude（Anthropic の Claude Fable 5.1 と Claude Opus 5）を使いました。正本の構成と、検査の仕組みの整備に使っています。本文は、Gemini CLI（指定は gemini-3.7-flash、実体は Google の gemini-3.5-flash）で書きました。正本の文に ID を振り、各節で使ってよい文だけを渡して、その命題から書き直す方式です。コードの抜粋は、実装から逐語で取りました。筆者が内容を確認し、必要に応じて修正しました。公開した内容の責任は筆者が負います。
