...
const slots = [
  ...
  { id: 'gate', secs: ['7.1', '7.3'], intent: '公開の門。スイッチは媒体ごとに1つで、生成AIを含め誰がブランチで立ててもよい。公開のワークフローの手動の起動も生成AIが行ってよい。人だけが通せる門は、人が行う main へのプルリクエストのマージ。公開のワークフローは main 以外のブランチからの起動を最初のジョブで止める。note と SNS の投稿のジョブは、使えるブランチを main だけに限った GitHub の Environment に紐づくので、止める段を含まない古いブランチのワークフローのファイルから起動しても止まる。Environment に紐づかない Qiita の同期は、そうしたファイルから起動すると止まらない。main にブランチの保護は無く、main へ直接 push すればマージを経ずに公開される。Qiita は検査を通らないと同期しない。Qiita の記事は公開で作り、未同期の記事も private: false なら最初の同期で公開される。Zenn は連携が直接公開するため検査で止まらない。5〜8 文。', min: 3 },
