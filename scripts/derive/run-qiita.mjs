...
const slots = [
  ...
  { id: 'gate', secs: ['7.1', '7.3'], intent: '公開の門。スイッチは媒体ごとに1つで、生成AIを含め誰がブランチで立ててもよい。SNS はマージの後、原稿の予定日の定期実行で投稿する(毎日 09:00 JST の予定で動き、予定日を過ぎていて期限内で、その媒体が有効で、台帳にその媒体の記録が無い ready の原稿を 1 本だけ投稿する。定期実行は遅れることも、その回が動かないこともある)。手動の起動は、予定日より前に出すときと、送信の前に失敗した原稿を送り直すときに使う。公開のワークフローの手動の起動も生成AIが行ってよい。人だけが通せる門は、人が行う main へのプルリクエストのマージ。公開のワークフローは main 以外のブランチからの起動を最初のジョブで止める。note と SNS の投稿のジョブは、使えるブランチを main だけに限った GitHub の Environment に紐づくので、止める段を含まない古いブランチのワークフローのファイルから起動しても止まる。Environment に紐づかない Qiita の同期は、そうしたファイルから起動すると止まらない。main にブランチの保護は無く、main へ直接 push すればマージを経ずに公開される。Qiita は検査を通らないと同期しない。Qiita の記事は公開で作り、未同期の記事も private: false なら最初の同期で公開される。Zenn は連携が直接公開するため検査で止まらない。6〜9 文。', min: 3 },
  { id: 'code_intro', secs: ['7.4', '9'], intent: 'コアコードの前置き。note 投稿は Playwright でエディタに流し込み公開ボタンを押す方式で、画面構造に依存する限界がある。投稿の対象は、マージで変わった原稿のうち status が ready か published のもの。新規か更新かは投稿の直前に main から読んだ最新の台帳で決め、内容が前回と同じなら何もせず、published で台帳に記録が無ければ投稿しない。3〜5 文。', min: 2 },
  ...
  { id: 'summary', secs: ['4.1', '7.1'], intent: 'まとめ。正本を1つに置き、派生物は別に書く。人だけが通せる門は、人が行う main へのマージ 1 つに置く。2〜3 文。', min: 2 },
];
