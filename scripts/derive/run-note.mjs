...
const slots = [
  ...
  { id: 'switch', heading: '公開の前に、必ず私の操作を1つ置く', secs: ['7.1', '7.4'], intent: '公開のスイッチは媒体ごとに1つで、生成AIを含め誰が立ててもよい。変更は本流とは別の枝に置き、公開の処理が動くのは、自分が本流へ取り込んだ(マージした)ときか、本流から手で起動したとき(手で起動するのは生成AIでもよい)。SNS だけは、原稿に書いた予定の日が来ると自動で投稿される(1 日 1 本まで。すでに出したものは出さない)。枝からの起動は公開の処理の最初の段で止める(この段を持たない古い枝から起動しても、noteとSNSの投稿は、本流からしか許さないGitHubの設定で止まる。止まらないのはQiitaの同期と、本流へ直接送った変更)。note は、取り込んだ変更に含まれる原稿のうち、印が ready か published のものが自動で反映される。どの原稿がどの記事になったかの記録があれば同じ記事を書き換え、記録が無ければ ready は新しく投稿し、published は投稿しない。一人称、平易な言葉で。4〜6 文。', min: 3 },
  { id: 'ai', heading: 'AIとの線引き', secs: ['7.1'], intent: '生成AIには公開の印を立てるところまで任せる。以前は、公開の手前に人の確認を重ねても公開は最後の門で止まるので、守りは増えず人の操作だけが増えている、と考えた（当時の理由）。そこで、確認の記録を本流への取り込み（マージ）の1つだけに残すと決めた。のちに、公開の処理を手で起動することも生成AIに任せた(中身は取り込みのときに確かめており、起動はそのあとの手順のため)。公開の処理は本流以外からの起動を止めるので、公開の前には、人の取り込み(マージ)が入る(止まらない経路は正本の 9 章)。3〜5 文。', min: 2 },
