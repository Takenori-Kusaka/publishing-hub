...
この問題の切り分けと対策の特定には、実機を用いた多角的な検証が必要であり、解消までに約2時間の試行錯誤を要しました。本稿では、子供向けのランチャーである「Google Kids Space」と Family Link の関係を調べます。さらに、当初の仮説と Android のショートカット固定 API の関係を説明し、Chrome のウェブ利用制限ポリシーが PWA インストール導線に与える影響の検証結果を示します。これらに基づき、運用上の推奨設計について整理します。

この記事の原稿は、次のリポジトリで管理しています。
...
Android OS では、ショートカットや PWA のアイコンをホーム画面に追加する際、ランチャー（Launcher）が対応しているかを確認するために、`ShortcutManager` API が用いられます。以下は、この API の呼び出し方を示すための例です。
