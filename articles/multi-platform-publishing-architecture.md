...
| SNS | 原稿の`status: ready`と対象コミットの`revision` | SNS公開（マージの後、予定日の定期実行。`main`での手動起動も可） |
...
公開は、マージの後に原稿の予定日（`publish_after`）で動きます。ワークフローは毎日午前9時（日本時間）を予定として動き、`main`の原稿から次の条件をすべて満たすものを1本だけ選んで投稿します。`status`が`ready`であること、`publish_after`が現在時刻以前であること、`expires_at`が現在時刻より後であること、その媒体が有効であること、台帳にその媒体の記録が1件もないことです。台帳は媒体ごとに見るため、LinkedInだけ投稿済みならBlueskyだけを投稿します。選ぶ順は`publish_after`の早いもので、1回の実行で投稿するのは1原稿までです。対象がなければ何もせず、ジョブは成功で終わります。投稿に使うコミットは、その時点の`main`の40桁のSHAです。定期実行は遅れることも、その回が動かないこともあります（9章）。2026年9月23日に筆者がこの定期実行を入れるまで、ワークフローは予定日を見ておらず、日付を守るのは起動する側でした。

手動の起動も残しています。予定日より前に出すときと、送信の前に失敗した原稿を送り直すときに使います。手動の起動では、確認欄に`PUBLISH`と完全一致で入力しなければ、準備ジョブの最初のステップで失敗します（定期実行には人の入力がないため、この確認は求めません）。`main`以外のブランチから起動した場合と、指定したコミットが40桁のSHAでない場合は、コミットを取り出す前に失敗します。定期実行は、GitHubの決まりで既定ブランチのワークフローのファイルだけが動きます。どちらの契機でも、同じジョブは続けて対象のコミットを取り出し、そのコミットが`main`に含まれること、原稿の`status`が`ready`であること、原稿に書かれた`revision`のコミットが履歴に存在することを確認します。実際に投稿するジョブはGitHubのEnvironment`social-production`に紐づき、SNSの公開のワークフローの中では、シークレットはこのジョブにだけ渡されます。同じEnvironmentは週次のトークンの監視（7.5節）でも使い、Blueskyのアプリパスワードはその監視のジョブにも渡ります。Environmentに必須レビュアーを設定すると、承認されるまでジョブはシークレットにアクセスできません（[GitHub Docs: Managing environments for deployment](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/manage-environments)）。この環境には必須レビュアーを置いていません。使えるブランチは`main`だけに限っています（9章）。以前の文書は、この承認をSNSの最後の門としていました（7e10c15、634d1b4）。しかし実行の記録には、承認を待った実行が1件もありません。SNSの公開の実行は、どれも承認の記録が空で、承認待ちの状態も記録されていません。承認の場所は、2026年9月16日に指示書の上でプルリクエストのマージへ移しました（7.1節）。
...
投稿の結果は、媒体ごとのJSON Lines形式の台帳に追記だけで記録します（Append-Only）。送信前に`pending`、成功で`published`、途中で失敗すると`partial`や`pending-unknown`を残します。台帳は`main`ではなく専用の`social-ledger`ブランチに置き、Zennの同期を汚しません。

投稿のジョブは、投稿の前にこのブランチの台帳を作業ツリーへ復元し、投稿の後に記録のジョブがブランチへ追記します。二重投稿の拒否は「媒体と原稿ID」で判断し、コミットのSHAは見ません。原稿を直すとSHAが変わるため、SHAを鍵にすると同じ原稿を二度投稿できてしまうからです。一度でもその媒体に届いた記録（`published`、`partial`、`pending-unknown`、人が解決した記録、人が媒体の側で消した記録）があれば、投稿しません。送信の前に失敗した`failed-before-send`だけは、手動の起動でそのまま送り直せます。届いた記録を越えて投稿するには、手動の起動に`--allow-repost`と12文字以上の理由を付けます。理由は台帳に残り、定期実行ではこの指定を使いません。届いた記録があると二度と投稿できない状態を、人が確かめたうえで抜けるための道です。定期実行は、記録が1件でもあれば選びません。状態の分からない記録や、送信の前に落ちた記録を、人が見ないまま自動で投稿し直さないためです。

復元と追記は2026年9月23日に入れました。それまで投稿のジョブは台帳を復元せず、記録のジョブはその実行の分だけの台帳でブランチのファイルを置き換えていました。そのため台帳には最後の実行の記録しか残らず、二重投稿の拒否は同じジョブの中でしか働きませんでした。実測では、ブランチの先頭の`bluesky.jsonl`が持つのは最後の実行の記録だけで、それ以前の4原稿ぶんの記録は履歴の中にしかありません。先頭から記録が落ちたのは9月22日の実行が最初で、このときは2原稿ぶんが落ちました。復元は、ブランチの履歴に一度でも現れた行を集める方式にしたので、置き換えで消えた記録も戻ります。
...
Qiitaの記事は`main`への反映でQiita CLIが同期します。同期の前に、Qiita向けの校正、レシピの要件、正本との重複率、複数称の検査を通します。同期のワークフローは実行を1つずつ動かし、その時点の最新の`main`を検査して同期します。先の実行が新しく作った記事のIDを`main`へ書き戻した後に、次の実行が同期するためです。検査の後に`main`のQiitaの記事が変わっていたら、その実行では同期せず、その変更で動く実行に任せます。

同期で付いた記事のIDと更新日時は、`main`へ書き戻します。書き戻しは、`main`の最新を取り直し、その上に`platforms/qiita`の変更だけを載せたコミットを作って押し戻す形です。作業ツリーとHEADは動かさず、押し戻す前に、親との差分が`platforms/qiita`の中だけであることを確かめます。同期のあいだに`main`が進んで押し戻しを拒まれたら、取り直して載せ直します。2026年9月23日までは、Qiita CLIのGitHub Actionsが同期の直後にコミットして押し戻していました。このアクションは拒まれてもやり直さずに失敗します。9月22日の実行で、直前にnoteの台帳のコミットが`main`へ入ったために押し戻しが拒まれ、新しく作った7本の記事のIDが残りませんでした。IDが残らないと、次の同期は同じ記事をもう一度作ります。このときは人がIDを手で書き戻して直しました。書き戻しに失敗したときは、残らなかったIDと更新日時をログに出し、ジョブを失敗させます。同期が途中で失敗したときも書き戻します。そこまでに作った記事のIDが`main`に残らないと、次の同期が同じ記事をもう一度作るからです。Qiita CLIは同期のたびに、Qiita上の記事の写しを`platforms/qiita/public/.remote`の下に書きます。これは原稿ではなく次の同期で作り直されるので、書き戻しの対象から外し、`main`には入れません。

記事は公開で作ります。まだ同期されていない記事（IDがない記事）も、`private: false`なら最初の同期で公開の状態でQiitaに置かれ、IDが付きます。`private: true`の記事は限定共有（URLを知っている人だけが読める状態）で置かれ、`ignorePublish: true`の記事は同期されません。以前は、最初の同期を限定共有にしてIDを付け、公開への切り替えをその後のマージで行う順序にしていました。本稿のQiita向け記事も、この順序で公開しました（限定共有で同期、IDの付与、人が公開へ切り替え）。この順序を検査として機械化したのは、その翌日です。未同期の記事に`private: true`か`ignorePublish: true`を求める検査でした。2026年9月22日に、筆者は「Qiitaの記事は公開で作る」と決め、この検査を外しました。検査の仕組みは残してあり、設定ファイル`lint/policies/qiita.json`の値を1つ戻せば、再び効きます。
...
| 2026-09-23 | 公開のワークフローの手動の起動を、人だけが行う操作から外した。生成AIが起動してよい（7.1節）。同じ日に、SNSの原稿1本を生成AIが起動して公開した。SNSを予定日の定期実行で投稿するようにし、投稿の前に台帳を復元して投稿の後に追記するようにした（7.2節）。QiitaのIDの書き戻しを、押し戻しを拒まれたらやり直す形に置き換えた（7.3節） |
...
- 公開台帳による二重投稿の拒否は、投稿の前に`social-ledger`ブランチの台帳を復元するので、実行をまたいでも動きます。残る穴は、記録のジョブが台帳を押し戻せなかった実行です。その実行の記録は残らないため、次の実行が同じ媒体へもう一度投稿しえます。押し戻せなかったときは、残らなかった記録をログに出してジョブを失敗させますが、直すのは人です。台帳の外で投稿したものも、機構は知りません。
...
- SNSの定期実行は、1回の実行で1原稿までしか投稿しません。予定日を過ぎた原稿が3本たまっていれば、すべて出るまでに3日かかります。予定日を待たずに出したいときは、手動で起動します（7.2節）。
- SNSの定期実行はGitHub Actionsの`schedule`で動きます。この契機は負荷の高い時間帯に遅れることがあり、負荷が十分に高ければ、順番待ちのジョブが取り消されることもあります（[GitHub Docs: Events that trigger workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows)）。本リポジトリの週次の定期実行は、毎週月曜の0時（協定世界時）に設定しています。2026年9月14日と9月21日の実行は、どちらも1時50分（協定世界時）に始まり、約1時間50分遅れていました。予定日の当日に出ること、午前9時（日本時間）に出ることは、どちらも保証されていません。遅れや欠測を知らせる仕組みはなく、予定日に出ていないことは人が見るまで分かりません。
- 台帳にその媒体の記録が1件でもある原稿は、定期実行が二度と選びません（7.2節）。送信の前に落ちた記録も、人が媒体の側で消した記録も、記録があることに変わりないからです。選ばなかったことは記録に残らないため、対象がなかった日と見分けがつきません。たとえば`quickscribe-floor-2026-09`は、LinkedInの投稿を人が消し、Blueskyは送信の前に落ちているため、定期実行の対象から外れたままです。出し直すには人が手動で起動します。送信の前に落ちた媒体はそのまま送り直せますが、一度届いた媒体は`--allow-repost`と12文字以上の理由を付けたときだけ投稿します。理由は台帳に残ります。
...
- Qiitaの同期は、新しく作った記事のIDを`main`へ書き戻します。同期の最中に`main`が進んで押し戻しを拒まれたときは、取り直して載せ直します（7.3節）。それでも押し戻せなかったときはIDが残らず、次の同期がその記事をもう一度作ります。既存の記事の`updated_at`も古いまま残るため、その記事を次に直したとき、Qiita CLIは原稿がQiita上の記事より古い可能性があるとして止まり、全記事の同期が止まります。どちらも、人がIDと更新日時を原稿へ書き戻すまで直りません。
- 配信予定日（`publish_after`）を未来に置いたnoteの原稿は、マージのときの実行では投稿されずに終わります。その日時を過ぎても、自動では投稿されません。定期的に動く実行が無いためです（定期実行があるのはSNSだけです）。その日時の後に、`main`から手動で起動するか、原稿を変えてマージすれば投稿されます。
...
次に取り組むのは、9章に挙げたEXIF除去の接続です。規則と原稿が同じリポジトリにあるので、直した結果はそのまま次の検査に反映されます。設計文書、検査規則、スクリプトはすべて[リポジトリ](https://github.com/Takenori-Kusaka/publishing-hub)にあります。
