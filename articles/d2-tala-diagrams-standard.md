...
### 1.1 Zennの本文幅と電話での縮小
Zennは、本文幅を超える画像を自動で縮小し、表示します。
本文幅は、PCでは約700px、幅390pxの電話では約360pxです。
この仕様によって、横長の図は縮小された際、文字の潰れが発生して読めなくなってしまいます。
2026年9月21日に、筆者がZennで公開している本を幅390pxで表示して測ったところ、自然幅690pxの図は0.52倍に縮み、図の中の16pxの文字は約8pxになりました。
そのため、掲載する図の自然サイズ（元の幅）を480px以下に設計します。
480px以下なら、電話でも0.75倍より縮まず、図の中の16pxの文字が約12px以上で読めます。
...
- **コンテナ（入れ子）の自動配置は弱い傾向にあります。** ノードの増加に伴って横方向へ際限なく広がるため、簡単にPCの本文幅（約700px）を超えてしまいます。
...
3. **自然幅の検査：** メタ情報の `natural.width` が 480px 以下に収まっているか。
...
## 5. 自然幅を480px以下に収めるためのプラクティス

CIのD5検証（自然幅が480pxを超えるとエラーで停止）に引っかかった場合、以下の設計アプローチを順に試して図をスリムにします。
