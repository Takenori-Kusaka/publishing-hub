...
「呼んでよい」と「呼べる」を分け、呼べるかどうかは実際に呼ぶまで確定しないと契約に書きます。部品は 3 つです。呼び出し先の抽象化、本番で 1 度も生成AIが成立していなかった事故から書き直した `isAvailable()` の契約、縮退先の規則です。
...
Bedrock に送るのは子供の識別子を含まない内容だけです。この約束を法務文書だけに置かず、外部の生成AIのライブラリを読み込んでよいファイルを許可一覧が固定する走査テストとして、コードの側でも表明しています。新しいファイルがライブラリを掴んだ時点でテストが落ち、「送る内容に子供の識別情報が入っていないか」を人が判断する契機になります。
...
権限、環境変数、モデルの識別子のすべてが正しく配られていました。それでも生成AIは 1 度も動いていませんでした。正本の原則 9 の言うとおり、値が正しいことと値が効いていることは別物で、両方をテストする必要があります。10 条の全体は [原則の章](https://zenn.dev/takenori_kusaka/books/ganbari-quest-design/viewer/principles) にあります。
