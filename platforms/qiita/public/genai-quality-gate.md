...
3. **公開の門をPRのマージに一本化する:**
   ...
   公開のワークフローは `main` へのpush（マージ）か手動の起動で動き、生成AIは `main` へ直接pushせず、手動の起動も人だけが行う決まりにしているからです。この決まりのもとでは、生成AIが立てたスイッチの後ろに、必ず人の操作が残ります。
   本格的な査読は、公開された記事で行います。表示は媒体ごとに違い、PRでは確かめられないためです。
   人の `Reviewed-by` トレーラーを公開の直前に確かめる検査（H1）も持っていますが、今は求めていません。公開の前に人のマージか人の手動の起動がある以上、同じ承認をもう一度求めることになるためです。
...
ZennはGitHub連携が `main` を公開し、Qiitaとnoteはワークフローが `main` へのpushか、人の手動の起動で動きます。
SNSの投稿だけは、マージのあとに人が `social-publish.yml` を手動で起動し、確認の語（`PUBLISH`）を入力して行います。
