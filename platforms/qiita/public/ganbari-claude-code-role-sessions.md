...
開発者は一人ですが、リポジトリには企画部、開発部、品質保証部、監査部、基盤部の 5 つの部署のセッションがあり、それぞれ別の複製リポジトリで動いています。部署は人ではなく、Claude Code のセッションに与えた役割です。一人なのに、なぜ部署を分けるのでしょうか。

生成AIは、自分で書いた変更を自分で承認して取り込んでしまいます。それを人の注意ではなく仕組みで止めるためです。実装を任せると実装の費用は下がりますが、検証と判断の費用は下がりません。作る側と認める側を、セッションと GitHub のアカウントの両方で分けました。この体制を回す仕組みは 3 つです。セッション間の受け渡しを GitHub のラベルに載せる仕組み、その経路の欠落を運用文書のテストで検出する仕組み、手元の検査を安い順に落として自動検査で確定する仕組みです。
