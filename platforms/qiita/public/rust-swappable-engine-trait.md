...
この記事の作成には、生成AIを2つ使いました。Gemini CLI（Google の gemini-3.5-flash）に、正本の章からQiitaのレシピへ組み替えるたたき台の作成（構成案と技術選定の比較表の下書き）を任せました。そのうえで Claude（Anthropic の Claude Opus 5）に、本文の全面的な書き直し、参照した公式ドキュメントのURLの実在確認と章番号の修正、コード抜粋の出典の付け直し、正本と重複する表現の削除を任せました。筆者が内容を確認し、必要に応じて修正しました。公開した内容の責任は筆者が負います。
