...
Aurora DSQL では主キーが物理的な配置で、作成後に変更できません。全テナント表の主キーは凍結の一覧（manifest）で固定し、一覧と設計書の表と Drizzle ORM の定義の 3 つが一致することを契約テスト（fitness function）が検査します。構造や文書と実装の一致を検査するこの種のテストを、正本にならってこう呼びます。API の契約テストとは別物です。
