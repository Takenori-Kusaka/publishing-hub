...
答えは、決まりそのものをテストに書き換えることです。構造や運用文書と実装の一致を検査するテストを、正本にならって契約テストと呼びます。Neal Ford らの fitness function の訳で、API の仕様を検証する契約テストとは別物です。新しい道具は入れず、既存の Vitest と Node.js のファイル走査だけで書けます。例は 4 つで、層の境界、データベースの窓口、不採用の記録、除外の理由です。
...
きっかけは書き出しと取り込みの機能でした。2 サイクル連続で止める指摘が出て、値の範囲を個別に当て布した結果「1 つ直すと次が露見する」状態になりました。診断は、再発の防止が人の注意に頼り、不変条件が画面操作テストにしか書かれていないという構造の失敗です。決めた原則は 5 つです。5 つ目は受け入れる残課題の関門で、残す課題を統合プルリクエストの本文に記録します。テストに関わる残りの 4 つを挙げます。
...
同じ走査の形で、違反の件数に上限を刻む変種もあります。デザインシステムの基礎の色トークンを画面の経路で直接使う既存の箇所が多数あり、0 件で止める検査は不可能でした。そこで現状の件数を上限にし、増えたら落ち、減らしたら定数を下げる歯止めにしています。
...
設計判断の記録の一覧には、調べたが採らなかった道具を並べた表があります。ある道具を不採用と記録した約 1 週間後に採用したのに、表は不採用のまま残り、読んだ人が「未採用」と判断する逆向きの機能を果たしていました。表は、採用したら採用記録へ移すと自分で定めていましたが、移動は人の注意に依存していて発火しませんでした。
...
リポジトリ全体を読むテストは、リポジトリが育つほど遅くなります。時間切れを既定の 5 秒のまま単体テストの列に置くと、並走する他のテストと CPU やファイル入出力を取り合い、負荷次第で落ちます。落ちた原因が回帰なのか負荷なのかは見ただけでは分からず、そのたびに切り分けの手間が発生します。
...
型止めを掛ける対象は、データの整合、課金、認可、日付の境界、表示の崩れのように顧客から見える不変条件に限ります。自動検査の関門やフックやプルリクエストの本文の検査、つまり検証の道具自身の不具合は対象外です。検証の道具に型止めを掛けると、道具を守るための道具が増え、増えた道具が次の不具合の型を生みます。道具の不具合に効くのは型止めではなく、道具を減らすことです。
