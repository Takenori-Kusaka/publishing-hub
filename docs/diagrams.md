...
**Zenn は本文幅より広い画像を縮小して表示します。** 本文幅は、PC では約 700px、幅 390px の電話では約 360px です。横長の図はそこで文字が潰れて読めません。2026 年 9 月 21 日に、公開中の本を幅 390px で表示して測ったところ、自然幅 690px の図は 0.52 倍に縮み、図の中の 16px の文字は約 8px になりました。だから図の自然幅は 480px 以下にします。電話でも 0.75 倍より縮まず、16px の文字が約 12px 以上で読めます。これは機械で検査できます（D5）。
...
3. 出力された自然幅を見る。480px を超えたら後述の方法で詰める
4. 本文から PNG を参照する
5. `npm run check` を通す
...
`npm run check:diagrams` の D5 が、自然幅が 480px を超えた図を**エラーで止めます**。超えたときの詰め方は次の順で試してください。
