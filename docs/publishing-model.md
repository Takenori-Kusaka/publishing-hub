...
5. **`main` 以外からの公開の起動を止める (`publish-qiita.yml` / `publish-note.yml` / `social-publish.yml`):**
   ...
   - 起動そのものは、人と AI のどちらが行ってもよい（2026-09-23 のオーナーの決定）。原稿の中身はマージのときに承認されており、起動はそのあとの機械的な操作だから（AGENTS.md 1 章）。人だけが通せる門は `main` への PR のマージ 1 つ。SNS の原稿の `campaign.publish_after` は投稿を止めないので、予定日を守るのは起動する側。
