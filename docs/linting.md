...
| Q12 | コードの抜粋は、先頭 3 行のコメントに書いた出典のファイルと逐語で一致する（行末コメントとコメント行も比べ、除くのは省略記号 `// ...` の行だけ）。出典に `@gate` の印がある安全のための分岐は、それより後の処理を載せるなら省かない。続けて並べた 2 行のあいだで出典の行（空行とコメント以外）を飛ばすなら `// ...` を置く（警告）。出典のないコードは警告 | エラー / 警告 |
...
| Q17 | 題名かタグに掲げた技術（GitHub Actions など。`qiita.json` の `topic_code`）の設定かコードを、出典のパス付きで 1 つ以上抜粋している | 警告 |
