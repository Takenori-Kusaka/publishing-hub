...
| N11 | 実装の語（Environment、ワークフロー、CI、YAML など）。note の読者に通じる言葉にする | 警告 |
...
| V8 | 派生物（Qiita / note / SNS）が正本の限界・但し書きと矛盾する記述をしている。テーマごとの照合リスト `lint/claims/<id>.json` と文単位で照合する（コードのコメント行も見る）。但し書きの語（unless）は、矛盾する語句の前後 30 字以内にあるときだけ効く | エラー |
